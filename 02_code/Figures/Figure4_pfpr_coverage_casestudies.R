...
tilep1 <- output |>
  ...
  filter(pfpr1 == pfpr) |>
  select(pfpr1, pfpr2, draw, case_avert_p, detect, infectious_prev, sens)
...
p1 <- ggplot(timepall, aes(x = factor(pfpr1), y = factor(pfpr2), fill = case_avert_p)) +
  ...
  scale_fill_gradientn(colors = met.brewer("Hiroshige"), 
                       values = c(-1, -0.08, 1),
                       breaks = seq(0, 14, 2)) + 
  scale_y_discrete(breaks = seq(0.05, 0.8, 0.1)) +
  scale_x_discrete(breaks = seq(0.05, 0.8, 0.1)) +
